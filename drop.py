...
def movePlatforms():
    ...
    for i, platform in enumerate(gamePlatforms):
...
def drawPlatforms():
    ...
    for platform in gamePlatforms:
        ...
        pygame.draw.rect(surface, (255,255,255), (platform["pos"][0], platform["pos"][1], windowWidth, 10)) # draw white platform
        pygame.draw.rect(surface, (0,0,0), (platform["gap"], platform["pos"][1], 40, 10)) # draw black gap

def gameOver():
    global gameStarted, gameEnded, platformsDroppedThrough
    print("Drops made: %i" % platformsDroppedThrough)
    platformSpeed = 0
    gameStarted = False
    gameEnded = True
